...
class device_status_controller {
	...
	public static function list_all($page = 1, $itemspp = 20) {
		...
		try {
			$device_status_list = device_status_model::list_all(($page - 1) * $itemspp, $itemspp);
			$ret = array();
			foreach($device_status_list as $device_status) {
				$ret[] = $device_status -> to_array_filtered($role);
			}
			return $ret;
		} catch(Exception $e) {
